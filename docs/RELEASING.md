# Release Checklist (browser-only fork)
...
- [ ] Rebuild macOS notifier helper with signing + notarization:
  ...
  - Signing inputs: `CODESIGN_ID="<your-developer-id-application>"` plus notary env vars `APP_STORE_CONNECT_API_KEY_P8`, `APP_STORE_CONNECT_KEY_ID`, and `APP_STORE_CONNECT_ISSUER_ID`.
  - Sparkle ed25519 private key should come from your own signing setup; export `SPARKLE_PRIVATE_KEY_FILE=<path-to-your-sparkle-key>` whenever the build script needs to sign an appcast/enclosure.
...
- [ ] Ensure README reflects the current browser-only CLI options and deprecation stubs.
...
4. **Validation**
  ...
  - [ ] Optional fast live browser smoke: `ORACLE_LIVE_TEST=1 pnpm test:live`
5. **Publish (npm)**
   ...
   - [ ] Use the `NPM_TOKEN` for your fork’s npm owner. If `npm publish` opens browser auth, the token wasn’t loaded; rerun with `source ~/.profile`.
   - [ ] Confirm auth: `npm whoami`.
   - [ ] Decide tag before publish:
     - If npm `latest` is ahead (e.g., `npm view <your-npm-package> version` shows a higher major), publish with `--tag legacy`.
     - If this should become latest, publish with `--tag latest` (or publish then `npm dist-tag add <your-npm-package>@X.Y.Z latest`).
   - [ ] `npm publish --access public --tag <legacy|latest>` (2FA OTP required even with token).
   - [ ] If promoting later: `npm dist-tag add <your-npm-package>@X.Y.Z latest --otp <code>` (OTP required).
   - [ ] `npm view <your-npm-package> version` (and optionally `npm view <your-npm-package> time`) to confirm the registry shows the new version.
   - [ ] Verify positional prompt still works: `npx -y <your-npm-package> "Test prompt" --preview`.
6. **Homebrew (tap)**
   - [ ] Update formula in `<path-to-your-homebrew-tap>/Formula/oracle.rb` (or create it if missing).
   ...
   - [ ] Verify install:
     ...
     - `brew tap <your-homebrew-tap> || true`
     - `brew install <your-homebrew-tap>/oracle`
     - `oracle --version`
...
- [ ] Confirm npm shows the new version: `npm view <your-npm-package> version` and `npx -y <your-npm-package>@X.Y.Z --version`.
- [ ] Promote desired dist-tag (e.g., `npm dist-tag add <your-npm-package>@X.Y.Z latest`).
...
- [ ] From a clean temp directory (no package.json/node_modules), run `npx <your-npm-package>@X.Y.Z "Smoke from empty dir" --preview` to confirm the package installs/executes via npx.
